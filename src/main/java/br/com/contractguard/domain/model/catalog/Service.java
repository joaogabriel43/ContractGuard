 ...
 *
 * <p>Identity is based solely on {@link #id}. Two services with the same id are considered the same
 * entity, regardless of other field values.
 *
 * <p><strong>Domain invariants:</strong>
 *
 * <ul>
 *   <li>{@code name} must not be null or blank.
 *   <li>{@code slug} must not be null or blank; it is always normalized to lowercase.
 * </ul>
 *
...
public class Service {
  ...
  /**
   * Creates a new {@code Service} with a generated UUID and the current timestamp. Use this when
   * registering a new service for the first time.
   */
  ...
  /**
   * Reconstitutes an existing {@code Service} from its persisted state. Use this when loading from
   * the database via an infrastructure adapter.
   */
