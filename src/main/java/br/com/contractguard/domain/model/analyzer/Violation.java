 ...
 *
 * <p>Immutable by design (Java record). Two {@code Violation}s are equal if all their fields are
 * equal — no identity-based equality.
 *
 * <p><strong>Domain invariants enforced in compact constructor:</strong>
 *
 * <ul>
 *   <li>{@code path} must not be null or blank.
 *   <li>{@code type} must not be null.
 *   <li>{@code severity} must not be null.
 *   <li>{@code message} must not be null or blank.
 *   <li>{@code httpMethod} is optional (may be null for non-endpoint-level violations).
 * </ul>
 *
...
public record Violation(
  ...
  /**
   * Returns {@code true} if this violation represents a breaking change that should cause a CI/CD
   * pipeline to fail.
   */
