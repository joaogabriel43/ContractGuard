...
public class AnalyzeContractUseCaseImpl implements AnalyzeContractUseCase {
  ...
  public AnalyzeContractUseCaseImpl(
  ...
  public DiffReport analyze(String serviceSlug, String newSpecContent, String version) {
    ...
    Service service =
        serviceRepositoryPort
            .findBySlug(serviceSlug)
            .orElseThrow(
                () ->
                    new DomainException(
                        String.format("Service with slug '%s' not found", serviceSlug)));

    // 2. Find baseline before saving the new one
    Optional<ApiSpecification> baselineOpt =
        apiSpecificationRepositoryPort.findLatestByServiceId(service.getId());

    // 3. Upload new candidate spec
    ApiSpecification candidateSpec =
        uploadSpecificationUseCase.upload(service.getId(), newSpecContent, version);
    ...
    if (baselineOpt.isPresent()) {
      ...
      report =
          DiffReport.createWithoutBaseline(
              service.getId(), candidateSpec.getId(), candidateSpec.getVersion());
    }
