                          ...
                          if (baselinePathItem.readOperationsMap().containsKey(httpMethod)) {
                            ...
                            if (candidateParams != null) {
                              for (Parameter candidateParam : candidateParams) {
                                boolean isRequired =
                                    candidateParam.getRequired() != null
                                        && candidateParam.getRequired();
                                if (isRequired) {
                                  boolean existsInBaseline =
                                      baselineParams != null
                                          && baselineParams.stream()
                                              .anyMatch(
                                                  p ->
                                                      p.getName().equals(candidateParam.getName())
                                                          && p.getIn()
                                                              .equals(candidateParam.getIn()));
                                  ...
                                  if (!existsInBaseline) {
                                    ...
                                    violations.add(
                                        Violation.of(
                                            ...
                                            String.format(
                                                "Required parameter '%s' (in %s) was added to %s %s",
                                                candidateParam.getName(),
                                                candidateParam.getIn(),
                                                httpMethod.name(),
                                                path)));
                                  }
                                }
