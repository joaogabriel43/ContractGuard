...
class SwaggerParserContractAnalyzerTest {
  ...
  @Mock private DiffRule rule1;

  @Mock private DiffRule rule2;
  ...
  void should_aggregate_violations_from_all_rules() {
    ...
    var v2 =
        Violation.of(
            "/users",
            "POST",
            ViolationType.REQUIRED_PARAM_ADDED,
  ...
  void should_throw_spec_parsing_exception_when_baseline_is_invalid() {
    assertThatThrownBy(
            () ->
                analyzer.analyze(
                    SERVICE_ID, UUID.randomUUID(), UUID.randomUUID(), INVALID_SPEC, SPEC_V2))
  ...
  void should_throw_spec_parsing_exception_when_candidate_is_invalid() {
    assertThatThrownBy(
            () ->
                analyzer.analyze(
                    SERVICE_ID, UUID.randomUUID(), UUID.randomUUID(), SPEC_V1, INVALID_SPEC))
        .isInstanceOf(SpecParsingException.class)
