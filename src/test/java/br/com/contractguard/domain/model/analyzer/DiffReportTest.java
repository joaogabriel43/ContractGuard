...
class DiffReportTest {
  ...
  private Violation breaking() {
    return Violation.of(
        ...
        "Endpoint GET /pets was removed");
  }

  private DiffReport reportWith(List<Violation> violations) {
    return DiffReport.create(
        SERVICE_ID, BASE_SPEC_ID, CANDIDATE_SPEC_ID, BASE_VERSION, CANDIDATE_VERSION, violations);
  }
  ...
  class FactoryValidation {
    @Test
    @DisplayName(
        "GIVEN no baseline WHEN createWithoutBaseline THEN creates empty successful report")
    void should_create_without_baseline() {
      var report =
          DiffReport.createWithoutBaseline(SERVICE_ID, CANDIDATE_SPEC_ID, CANDIDATE_VERSION);
    ...
    void should_throw_when_candidate_id_is_null() {
      assertThatNullPointerException()
          .isThrownBy(
              () ->
                  DiffReport.create(
                      SERVICE_ID, BASE_SPEC_ID, null, BASE_VERSION, CANDIDATE_VERSION, List.of()));
    }
  }
