...
class ContractGuardE2ETest extends AbstractIntegrationTest {
  ...
  @Autowired private ApiSpecificationJpaRepository apiSpecificationRepository;

  @Autowired private DiffReportJpaRepository diffReportRepository;
  ...
  void should_register_new_service() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/services").contentType(MediaType.APPLICATION_JSON).content(jsonPayload))
  ...
  void should_upload_baseline_spec_and_return_no_breaking_changes() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/services/" + SERVICE_SLUG + "/analyze")
